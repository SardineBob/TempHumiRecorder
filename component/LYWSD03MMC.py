    ...
    while True:
        try:
            ...
            if mea.sensorname in previousMeasurements:
                ...
                # only send data when it has changed or X identical data has been skipped, ~10 packets per minute, 50 packets --> writing at least every 5 minutes
...
def keepingLEScanRunning():  # LE-Scanning gets disabled sometimes, especially if you have a lot of BLE connections, this thread periodically enables BLE scanning again
    ...
    while True:
        ...
        if now - lastBLEPaketReceived > args.watchdogtimer:
            print("Watchdog: Did not receive any BLE Paket within", int(
                now - lastBLEPaketReceived), "s. Restarting BLE scan. Count:", BLERestartCounter)
            disable_le_scan(sock)
    ...
    def handleNotification(self, cHandle, data):
        ...
        try:
            ...
            if args.TwoPointCalibration:
                humidityCalibrated = calibrateHumidity2Points(
                    humidity, args.offset1, args.offset2, args.calpoint1, args.calpoint2)
                print("Calibrated humidity: " + str(humidityCalibrated))
                measurement.calibratedHumidity = humidityCalibrated
...
parser.add_argument("--battery", "-b", help="Get estimated battery level, in ATC-Mode: Get battery level from device",
                    metavar='', type=int, nargs='?', const=1)
parser.add_argument("--count", "-c", help="Read/Receive N measurements and then exit script", metavar='N', type=int)
parser.add_argument("--interface", "-i", help="Specifiy the interface number to use, e.g. 1 for hci1",
...
rounding.add_argument(
    "--round", "-r", help="Round temperature to one decimal place (and in ATC mode humidity to whole numbers)", action='store_true')
...
callbackgroup.add_argument("--callback", "-call",
                           help="Pass the path to a program/script that will be called on each new measurement")
callbackgroup.add_argument("--httpcallback", "-http",
                           help="Pass the URL to a program/script that will be called on each new measurement")
...
callbackgroup.add_argument(
    "--influxdb", "-infl", help="Optimize for writing data to influxdb,1 timestamp optimization, 2 integer optimization", metavar='N', type=int, default=0)

atcgroup = parser.add_argument_group("ATC mode related arguments")
atcgroup.add_argument(
    "--atc", "-a", help="Read the data of devices with custom ATC firmware flashed, use --battery to get battery level additionaly in percent", action='store_true')
atcgroup.add_argument("--watchdogtimer", "-wdt", metavar='X', type=int,
...
atcgroup.add_argument("--onlydevicelist", "-odl",
                      help="Only read devices which are in the device list file", action='store_true')
...
if args.mqttconfigfile:
    ...
    # MQTTS parameters
    if tls:
        client.tls_set(cacerts, certificate, certificate_key, cert_reqs=ssl.CERT_REQUIRED,
            ...
            if(atcIdentifier == "1A18") and not args.onlydevicelist or (atcIdentifier == "1A18" and mac in sensors) and (len(atcData_str) == 26 or len(atcData_str) == 16 or len(atcData_str) == 22):  # only Data from ATC devices
                ...
                if lastAdvNumber == None or lastAdvNumber != advNumber:
                    ...
                    if len(atcData_str) == 26:  # ATC1441 Format
                        ...
                        temperature = int.from_bytes(bytearray.fromhex(
                            atcData_str[12:16]), byteorder='big', signed=True) / 10.
                        ...
                        print("BLE packet: %s %02x %s %d" % (mac, adv_type, data_str, rssi))
                        temperature = int.from_bytes(bytearray.fromhex(
                            atcData_str[12:16]), byteorder='little', signed=True) / 100.
                        humidity = int.from_bytes(bytearray.fromhex(
                            atcData_str[16:20]), byteorder='little', signed=False) / 100.
                        batteryVoltage = int.from_bytes(bytearray.fromhex(
                            atcData_str[20:24]), byteorder='little', signed=False) / 1000.
                        batteryPercent = int.from_bytes(bytearray.fromhex(
                            atcData_str[24:26]), byteorder='little', signed=False)

                    # encrypted: length 22/11 Bytes on custom format, 16/8 Bytes on ATC1441 Format
                    elif len(atcData_str) == 22 or len(atcData_str) == 16:
                        # print("enc") # debug
                        ...
                        if lastData == None or lastData != atcData_str:
                            print("Encrypted BLE packet: %s %02x %s %d, length: %d" %
                                  (mac, adv_type, data_str, rssi, len(atcData_str)/2))
                            if mac in sensors and "key" in sensors[mac]:
                                ...
                                # lengthHex="0b"
                                ret = cryptoFunctions.decrypt_aes_ccm(
                                    bindkey, macReversed, bytes.fromhex(lengthHex + "161a18" + atcData_str))
                ...
                if mac in sensors:
                    ...
                    if "offset1" in sensors[mac] and "offset2" in sensors[mac] and "calpoint1" in sensors[mac] and "calpoint2" in sensors[mac]:
                        measurement.humidity = calibrateHumidity2Points(humidity, int(sensors[mac]["offset1"]), int(
                            sensors[mac]["offset2"]), int(sensors[mac]["calpoint1"]), int(sensors[mac]["calpoint2"]))
